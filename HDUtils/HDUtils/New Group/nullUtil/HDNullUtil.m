...
+(BOOL) checkNulltips:(NSArray<NSString *> *) tips isStrict:(BOOL) isStrict objects:(NSObject *) object,...{
    ...
    if (object) {
        NSObject *otherObj;
        while (index < tips.count && otherObj != [NSNull null]) {
...
+(void)defaultValue:(NSArray<NSObject *> *)defaultValues isStrict:(BOOL)isStrict objects:(NSObject *)object, ...{
    ...
    if (object) {
        NSObject *otherObj;
        while (index < defaultValues.count && otherObj != [NSNull null]) {
            //va_arg: 第二个参数是获取了类型。然后返回这个指定类型的值
            //并把args的位置指向变参表的下一个变量位置
            otherObj = va_arg(args, NSObject *);
...
+(NSInteger) numOfNullObjectIsStrict:(BOOL)isStrict objects:(NSObject *)object, ...{
    ...
    if (object == nil) {
        nullTotal += 1;
    }else if (isStrict && [object isKindOfClass:[NSString class]] && [((NSString *)object) isEqualToString:@""]) {
        nullTotal += 1;
    }
    ...
    NSObject *otherObj;
    while (otherObj != [NSNull null]) {
        //va_arg: 第二个参数是获取了类型。然后返回这个指定类型的值
        //并把args的位置指向变参表的下一个变量位置
        otherObj = va_arg(args, NSObject *);
        if (otherObj == nil) {
            nullTotal += 1;
        }else if (isStrict && [otherObj isKindOfClass:[NSString class]] && [((NSString *)otherObj) isEqualToString:@""]) {
            nullTotal += 1;
        }
